...
(()=>{
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = 'arena assist tool - v1.2.2d.パクリ5l';
  ...
  async function fetchSingleArenaInfo(elm) {
    try {
      ...
      const url = `https://donguri.5ch.net/teambattle?r=${row}&c=${col}&m=l`;
  ...
  async function autoJoin() {
    ...
    async function equipChange (region) {
      ...
      try {
        const res = await fetch(url);
        if(!res.ok) throw new Error(`[${res.status}] /teambattle?r=${row}&c=${col}&m=l}`);
        const text = await res.text();
        const doc = new DOMParser().parseFromString(text,'text/html');
        const h1 = doc?.querySelector('h1')?.textContent;
        if(h1 !== 'どんぐりチーム戦い') return Promise.reject(`title.ng`);
