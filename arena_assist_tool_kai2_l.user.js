...
(()=>{
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = 'arena assist tool - v1.2.2d.パクリ8l';
  ...
  async function fetchSingleArenaInfo(elm) {
    try {
      ...
      const url = `https://donguri.5ch.net/teambattle?m=l&r=${row}&c=${col}`;
  ...
  async function autoJoin() {
    ...
    async function getRegions () {
      try {
        const res = await fetch('');
        if (!res.ok) throw new Error(`[${res.status}] /teambattle?m=l`);
        const text = await res.text();
        const doc = new DOMParser().parseFromString(text, 'text/html');
        const h1 = doc?.querySelector('h1')?.textContent;
        if (h1 !== 'どんぐりチーム戦い') throw new Error('title.ng info');
    ...
    async function challenge (region) {
      ...
      const body = `m=l&row=${row}&col=${col}`;
      try {
    ...
    async function equipChange (region) {
      ...
      try {
        const res = await fetch(url);
        if(!res.ok) throw new Error(`[${res.status}] /teambattle?m=l&r=${row}&c=${col}}`);
        const text = await res.text();
        const doc = new DOMParser().parseFromString(text,'text/html');
        const h1 = doc?.querySelector('h1')?.textContent;
        if(h1 !== 'どんぐりチーム戦い') return Promise.reject(`title.ng`);
  ...
  async function drawProgressBar(){
    try {
      ...
      progressBarInfo.textContent = `[ﾗﾀﾞｰ] 第 ${currentPeriod} 期${str}`;
